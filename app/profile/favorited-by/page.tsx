...
export default function FavoritedByPage() {
  ...
  useEffect(() => {
    ...
    if (status === 'authenticated' && session?.user) {
      setLoading(true);
      setError(null);
      fetch('/api/favorites?type=favorited-by', {
        ...
        .catch(() => {
          setUsers([]);
          setError('Não foi possível carregar quem te favoritou.');
        })
    ...
    return (
      <>
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-[#18181b]">
          ...
          <span className="text-white text-lg">Carregando quem te favoritou...</span>
        </div>
      </>
    );
  }
        ...
        <div className="w-full max-w-2xl mx-auto bg-[#232326] rounded-lg p-6 shadow-lg mt-6">
          ...
          {users.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16">
              <span className="text-pink-400 text-2xl mb-2">Ninguém te favoritou ainda</span>
              <span className="text-gray-400">Quando alguém te favoritar, aparecerá aqui.</span>
            </div>
          ) : (
